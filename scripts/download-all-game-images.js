...
// Создаем папку для изображений игр
const imagesDir = path.join(__dirname, '..', 'public', 'game-images');
if (!fs.existsSync(imagesDir)) {
    fs.mkdirSync(imagesDir, { recursive: true });
}

// Полный список всех игр из casino page
const games = [
    {
    ...
    return new Promise((resolve, reject) => {
        ...
        https.get(url, (response) => {
            if (response.statusCode === 200) {
                ...
                console.error(`Ошибка скачивания ${filename}: ${response.statusCode}`);
...
async function downloadAllImages() {
    console.log(`Начинаем скачивание ${games.length} изображений...`);
    console.log(`Папка назначения: ${imagesDir}`);
    
    for (let i = 0; i < games.length; i++) {
        const game = games[i];
        const filename = `${sanitizeFileName(game.name)}.jpg`;
        
        try {
            console.log(`[${i + 1}/${games.length}] Скачиваем: ${game.name}`);
            await downloadImage(game.img, filename);
            // Небольшая задержка между запросами
            await new Promise(resolve => setTimeout(resolve, 200));
        } catch (error) {
            console.error(`Не удалось скачать ${game.name}:`, error.message);
        }
    }
    
    console.log('Скачивание завершено!');
    
    // Создаем JSON файл с маппингом
    const mapping = games.map(game => ({
        name: game.name,
        filename: `${sanitizeFileName(game.name)}.jpg`,
        localPath: `/game-images/${sanitizeFileName(game.name)}.jpg`,
        originalUrl: game.img,
        provider: game.provider,
        category: game.category
    }));
    
    fs.writeFileSync(
        path.join(imagesDir, 'games-mapping.json'), 
        JSON.stringify(mapping, null, 2)
    );
    
    console.log('Создан файл маппинга: games-mapping.json');
}
